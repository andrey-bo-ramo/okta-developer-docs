    ...
    >
      <i
        :class="{
          ...
          opened:
            (link.subLinks && sublinksExpanded) || isCurrentPage(link.path),
        }"
      >
        <svg
          width="5"
      ...
      <router-link :to="link.path" v-slot="{ href, navigate }">
        <a :href="href" @click="navigate" class="link" :title="link.title">
          <slot>
            <span class="text-holder">
              {{ link.title }}
            </span>
...
export default {
    ...
    entityType: function () {
      if (this.link.hasOwnProperty("path")) {
        ...
        if (this.link.path !== null) {
          if (
            this.link.hasOwnProperty("subLinks") &&
            this.link.subLinks.length > 0
          ) {
  ...
  methods: {
    ...
    setData: function () {
      ...
      let borderedSections = document.querySelectorAll(
        ".tree-nav .sections.bordered"
      );
      borderedSections.forEach((el) => {
        el.classList.remove("bordered");
      });
      let lastExpandedSection = document.querySelectorAll(
        ".tree-nav li.subnav-active > .sections"
      );
      if (lastExpandedSection.length) {
        lastExpandedSection[lastExpandedSection.length - 1].classList.add(
          "bordered"
        );
      }
    },
    ...
    isHeaderMenu: function () {
      return (function loop(parent) {
        ...
        if (parent.$el.classList.contains("page-header")) {
          return true;
        }
